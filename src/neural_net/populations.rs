...
use super::{
    nets::{MutationParams, Net, NetParams},
    species::{AllSpecies, SpeciesMetaParams},
};
    ...
    Fit: FitnessInfo,
{
    ...
    pub fn run_one_generation(
        &mut self,
        mutation_multipier: f64,
        fitness_of_net: impl FnMut(&mut Net<Fit>, f32) -> Fit,
    ) {
    ...
    pub fn create_initial_population(&mut self) {
        ...
        while self.nets.len() < self.population_params.population_size {
            ...
            net.mutate_self(
                &self.population_params.mutation_params,
                1.0,
    ...
    pub fn evaluate_population(&mut self, mut f: impl FnMut(&mut Net<Fit>, f32) -> Fit) {
        for net in self.nets.iter_mut() {
            let net_count_in_same_species = self
                .species
                .get(net.species_index.unwrap())
                .stats
                .current_count as f32;
            assert!(net_count_in_same_species >= 1.0);
            let fitness_info = f(net, net_count_in_same_species);
            assert!(
                (fitness_info.get_fitness() / net_count_in_same_species
                    - fitness_info.get_species_weighted_fitness())
                .abs()
                    < 0.001
            );
        ...
        self.species
            .recompute_species_stats_for_new_generation(&mut self.nets); // self.nets is now sorted!
    }
    ...
    pub fn create_next_generation(&mut self, mutation_multiplier: f64) {
        ...
        let mut map_species_index_to_net_infos =
            HashMap::<SpeciesIndex, Vec<NetInfoPerSpecies<Fit>>>::new();
        ...
        for (_, net_info_list) in
            map_species_index_to_net_infos
                .iter_mut()
                .filter(|(_, info_list)| {
                    info_list.len() >= self.population_params.min_required_members_to_forward_best
                })
        {
        ...
        for (&species_index, net_info_list) in map_species_index_to_net_infos.iter_mut() {
            ...
            let trunc = if should_eliminate_species {
                0
            } else if should_keep_most_of_species {
                (len - 1).max(0)
            } else {
                let trunc =
                    len as f32 * (1.0 - self.population_params.species_param.frac_eliminated);
        ...
        for (&species_index, net_info_list) in map_species_index_to_net_infos.iter_mut() {
            let sum_this_species = net_info_list.iter().fold(0.0, |acc, n| {
                acc + n.fitness_info.get_species_weighted_fitness()
            });
        ...
        for (&species_index, net_info_list) in map_species_index_to_net_infos.iter() {
            ...
            if net_info_list.len() == 1 {
                ...
                while nets_new.len() < target {
                    let net_chosen_a = self.choose_from(
                        net_info_list,
                        self.population_params.frac_chance_to_cross_globally,
                    );
                    let net_chosen_b = self.choose_from(
                        net_info_list,
                        self.population_params.frac_chance_to_cross_globally,
                    );
                    if std::ptr::addr_eq(net_chosen_a, net_chosen_b) {
                        continue;
                    } // Skip if same
                    let net_new = net_chosen_a.cross_into_new_net(
                        net_chosen_b,
                        &self.population_params.mutation_params,
                        mutation_multiplier,
                        species.stats.current_count as f32,
                    );
                    nets_new.push(net_new);
                }
            }
        ...
        if nets_new.len() < self.population_params.population_size - 30 {
            println!(
                "NEED MORE POPULATION: cur={}, target={}, sum_fracs={sum_fracs}",
                nets_new.len(),
                self.population_params.population_size
            );
        }
        while nets_new.len() < self.population_params.population_size {
            ...
            let net_new = net_chosen_a.cross_into_new_net(
                ...
                &self.population_params.mutation_params,
                mutation_multiplier,
                self.population_params.population_size as f32,
            );
            nets_new.push(net_new);
        }
        self.nets = nets_new;
    ...
    fn choose_from(
        &self,
        net_info_list: &[NetInfoPerSpecies<Fit>],
        frac_chance_to_cross_globally: f64,
    ) -> &Net<Fit> {
