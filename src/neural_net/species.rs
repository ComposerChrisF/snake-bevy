...
impl SpeciesMetaParams {
    pub fn compute_compatibility_distance<Fit>(&self, net1: &Net<Fit>, net2: &Net<Fit>) -> (bool, f32) where Fit: FitnessInfo {
        let n_nodes = net1.nodes.len().max(net2.nodes.len());
        let n_conn  = net1.connections.len().max(net2.connections.len());
        let n = (n_nodes + n_conn + 1 - (net1.net_params.input_count + net1.net_params.output_count)) as f32;
        ...
        let compatibility_distance = (self.c1_excess * excess_count) / n 
            ...
            + self.c3_weights * w;
        //println!("threshold={} vs. distance={compatibility_distance} (excess={excess_count}, disjoint={disjoint_count}, w={w}, n={n}, n_node={n_nodes}, n_conn={n_conn})", self.threshold);
        let is_same_species = compatibility_distance <= self.threshold;
...
'all_nets: 
        for net in nets.iter_mut() {
            ...
            for s in new_species_list.iter_mut() {
                let (is_in_species, _) = self.meta.compute_compatibility_distance(&s.representative, net);
                if is_in_species { 
            ...
            for s_old in old_species_list.iter_mut().filter(|s| s.is_extinct /* i.e. not already copied to new-species_list */) {
                let (is_in_species, _) = self.meta.compute_compatibility_distance(&s_old.representative, net);
                if is_in_species {
        ...
        for s in old_species_list.drain(0..) {
            if s.is_extinct { self.extinct_species_list.push(s); }
        }
...
//
        ...
        nets.sort_by(|a,b| std::cmp::Ordering::reverse(a.fitness_info.get_species_weighted_fitness().partial_cmp(&b.fitness_info.get_species_weighted_fitness()).unwrap()));
        assert!(nets[0].fitness_info.get_species_weighted_fitness() >= nets[nets.len() - 1].fitness_info.get_species_weighted_fitness());
        assert!(nets[0].fitness_info.get_species_weighted_fitness() >= nets[1].fitness_info.get_species_weighted_fitness());
        ...
        for s in self.species_list.iter_mut() {
            ...
            stats.current_sw_fitness_ave = stats.current_sw_fitness_sum / stats.current_count as f32;
